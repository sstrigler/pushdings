...
websocket_init(_Prot, Req0, _Opts) ->
    case cowboy_req:parse_header(<<"sec-websocket-protocol">>, Req0) of
        {ok, undefined, Req1} ->
            pushdings:debug("no protocols given, asuming default", []),
            Req2 = cowboy_req:compact(Req1),
            {ok, Req2, ?PROTOCOL, hibernate};
        {ok, Subprotocols, Req1} ->
            pushdings:debug("protocols: ~p", [Subprotocols]),
            case lists:member(?PROTOCOL, Subprotocols) of
                ...
                false ->
                    pushdings:warning("No supported protocol found in ~p",
                                  [Subprotocols]),
                    {shutdown, Req1}
            end
    end.
...
websocket_handle({text, Data}, Req, ?PROTOCOL = State) ->
    pushdings:debug("Got data: ~p", [Data]),
    #{ app_id     := AppId,
       user_id    := UserId,
        ...
        true ->
            case is_valid_user(AppId, UserId, UserToken) of
                true  ->
                    ...
                    {ok, Req, State};
                false ->
                    pushdings:warning("Could not auth user: ~p", [Json]),
                    {shutdown, Req, State}
            end;
        false ->
            pushdings:warning("App ~p rate limited", [AppId]),
            {shutdown, Req, State}
    end;
websocket_handle(Frame, Req, State) ->
    pushdings:warning("Not handling unknown frame: ~p", [Frame]),
    {ok, Req, State}.

websocket_info({gproc_ps_event, {pushdings, _Topic}, Message}, Req, State) ->
    pushdings:debug("pushing ~p", [Message]),
    {reply, {text, jsx:encode(Message)}, Req, State};
websocket_info(Info, Req, State) ->
    pushdings:warning("unknown message received ~p~n", [Info]),
    {ok, Req, State}.
